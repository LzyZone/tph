...
abstract class Driver {
     ...
     * @param boolean $reconnect 是否重连
     * @return void
     */
    protected function initConnect($master=true,$reconnect=true)
    {
        ...
        if ($reconnect) {
            try {
                ...
                $_index = 0;
                foreach ($this->linkID as $_index => $_linkID) {
                    if ($this->_linkID instanceof $_linkID) {
                        break;
                    }
                }
                $this->linkID[$_index] = null;
                $this->_linkID = null;
                $cnt = 3;
                if ($pex->getCode() == 'HY000' || stripos($pex->getMessage(), 'server has gone away') !== false) {
                    while ($cnt > 0) {
                        $cnt--;
                        $this->initConnect($master, false);
                    }
                }
